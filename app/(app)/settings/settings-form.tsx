...
import { Key, Save, Trash2, CheckCircle2, ExternalLink } from "lucide-react"
...
import { saveKeyAction, removeKeyAction } from "./actions"
...
export function SettingsForm({ hasKey, maskedKey }: SettingsFormProps) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await saveKeyAction(apiKey.trim())
      if (result.success) {
        toast.success("API-ключ успешно сохранён")
        setApiKey("")
      } else {
        toast.error(result.error || "Не удалось сохранить ключ")
      }
    })
  }
