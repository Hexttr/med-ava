...
export async function POST(request: NextRequest) {
  try {
    ...
    const negativeSuffix = negativePrompt
      ? ` ${negativePrompt} Avoid: unchanged source photo, direct background replacement, source image transplant, copied cutout edges, copied original clothing, copied original lighting, or a minimally edited source portrait.${styleNegativeSuffix}`
      : ` Avoid: unchanged source photo, direct background replacement, source image transplant, copied cutout edges, copied original clothing, copied original lighting, or a minimally edited source portrait.${styleNegativeSuffix}`
    ...
    function getAttemptCorrection(attempt: number): string {
      ...
      if (attempt <= 1) return ""
      if (attempt === 2) return ` ${SECOND_PASS_REWRITE_RULES}${medicalRetry}`
      return ` ${SECOND_PASS_REWRITE_RULES} ${FINAL_PASS_REWRITE_RULES}${medicalRetry}`
    }
